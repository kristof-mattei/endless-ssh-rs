    ...
    loop {
        tokio::select! {
            ...
            Some(expired) = clients.next() => {
                let overdue = Instant::now().duration_since(expired.deadline());
                let client = expired.into_inner();

                event!(Level::TRACE, addr = ?client.addr(), ?overdue, "Client due");

                let Some(client) = process_client(client, delay, max_line_length, &statistics_sender).await else {
